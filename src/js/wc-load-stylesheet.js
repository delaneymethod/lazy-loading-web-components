	...
	return new Promise( (resolve, reject) => {
		...
		if (! existingStylesheet) {
			if (params.href) {
				...
				reject(`WcLoadStylesheet -> ${params.id} -> missing href`);
			}
		} else {
			resolve(`WcLoadStylesheet -> ${params.id} -> already exists`);
		}
	}).catch(console.error);
};

global.WcLoadStylesheet = WcLoadStylesheet;
